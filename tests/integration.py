...
class Test:
    ...
    def test_integration(self):
        ...
        assert len(self.browser.find_elements_by_css_selector('li', min=4, timeout=1)) == 4
        ...
        assert len(self.browser.find_elements_by_css_selector('li', min=4, debounce=0.5)) == 5

        # Get us back to in sync with the 6th li
        assert len(self.browser.find_elements_by_css_selector('li', min=6, timeout=1)) == 6

        # Now make sure that even with no timeout, we run the recover and find 8 after recover is run
        assert len(self.browser.find_elements_by_css_selector('li', min=7, recover=self.sleep2)) == 8
        assert self.sleep_calls == 1

        # Finally, with no timeout, recover should only be run once more and finish quickly
        with pytest.raises(NoSuchElementException):
